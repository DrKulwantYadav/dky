...
export default async function LibraryArticle({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const topic=getLibraryTopic(slug);if(!topic)notFound();const related=libraryTopics.filter(item=>item.slug!==topic.slug).slice(0,2);const path=`/health-library/${topic.slug}`;return <main className="library-article-page">
  <JsonLd data={[medicalWebPageSchema({name:topic.title,description:topic.summary,path,about:topic.category}),breadcrumbSchema([{name:"Home",path:"/"},{name:"Health library",path:"/health-library"},{name:topic.title,path}]),{"@context":"https://schema.org","@type":"Article",headline:topic.title,description:topic.summary,mainEntityOfPage:`${siteUrl}${path}`,author:{"@id":physicianId},publisher:{"@id":physicianId},isAccessibleForFree:true}]} />
  ...
  <article className="library-article"><header className="article-hero"><a href="/health-library" className="back-link">← All health topics</a><p className="eyebrow"><span/> {topic.category}</p><h1>{topic.title}</h1><div className="article-meta"><span>Patient guide</span><span>{topic.readTime}</span><a href="/about-dr-kulwant-yadav">Written and medically reviewed by Dr. Kulwant Yadav</a></div><p className="article-intro">{topic.introduction}</p></header>
