...
export default function SetupPasswordPage() {
  ...
  return <main className="admin-login-page"><section className="admin-login-card"><div className="admin-brand-mark">KY</div><p className="admin-eyebrow">Secure Admin access</p><h1>Create your password</h1><p>Set a secure password for the Dr Kulwant Yadav Admin CRM.</p>{message&&<div className="admin-alert error">{message}</div>}{ready?<form className="admin-form" onSubmit={submit}><label>New password<input name="password" type="password" autoComplete="new-password" minLength={10} required/></label><label>Confirm password<input name="confirm" type="password" autoComplete="new-password" minLength={10} required/></label><button className="admin-primary-button">Save password and continue</button></form>:!message&&<div className="admin-alert">Checking your secure recovery link…</div>}</section></main>
}
